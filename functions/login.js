...
(async () => {
  ...
  chalk.infoFN("Menekan tombol MASUK (Membuka Popup)...");
  const isMasukClicked = await page
    .evaluate(() => {
  ...
  if (!isMasukClicked) {
    chalk.infoFN("Tombol MASUK tidak ditemukan. Sepertinya sudah absen masuk!");
    if (credentials.WEBHOOK_URL)
      discord.alreadyAbsenNotification(new Date(), true);
  ...
  chalk.infoFN("Menunggu 2 detik sebelum menekan MASUK...");
  await page.waitForTimeout(2000);

  chalk.infoFN("Menekan tombol MASUK (Kirim Absen) di dalam Popup...");
  await page.evaluate(() => {
  ...
  chalk.infoFN(
    "Absensi MASUK telah dikirim! Menunggu 5 detik untuk konfirmasi akhir..."
  );
  await page.waitForTimeout(5000);
  ...
  if (credentials.WEBHOOK_URL) discord.successLogin(new Date());
